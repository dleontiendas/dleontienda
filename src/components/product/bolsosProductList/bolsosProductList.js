...
/* ---------- Main (solo BOLSOS) ---------- */
export default function BolsosProductList() {
  const { products, loading, error } = useContext(ProductsContext);

  // Filtra solo categoría "Bolsos" (tolerante a acentos/idiomas)
  const bolsosBase = useMemo(() => {
  const isBolso = (value = "") => {
  ...
  const enriched = useMemo(
    () => bolsosBase.map((p) => ({ ...p, depSlug: depToSlug(p) })),
    [bolsosBase]
  );
    ...
    <div className="container product-list-container">
      ...
      <div className="product-grid">
        {sorted.length ? (
          ...
          <p className="center-align">No se encontraron productos de Bolsos</p>
        )}
      </div>
    </div>
  );
