  ...
  return (
    <div className="col s12 m6 l4 xl3">
      <div className="card product-card-airbnb">
        <Link
          to={`/products/${product.catSlug || "moda"}/${product.id}`}
  ...
  const modaBase = useMemo(() => {
    const isModa = (cat = "") => {
      ...
      return (
        c.includes("moda") ||
        c.includes("ropa") ||
        c.includes("vestuari") || // vestuario/vestuarios
        c.includes("fashion") ||
        c.includes("apparel") ||
        c.includes("clothing") ||
  ...
  {subcatGroups.map((g) => {
    ...
    return (
      ...
      <div className="product-grid">
        {sorted.length ? (
          sorted.map((p) => (
            <ProductCard key={`${p.catSlug || "moda"}-${p.id}`} product={p} />
          ))
        ) : (
          <p className="center-align">No se encontraron productos de moda</p>
        )}
      </div>
    </div>
  );
